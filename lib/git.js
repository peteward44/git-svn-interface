...
function executeGit( args, options ) {
	...
	return new Promise( function( resolve, reject ) {
		var stdo = '';
		var proc = doSpawn( 'git', args, { cwd: options.cwd ? options.cwd : process.cwd() } );
...
export async function exists( url, targetDesc, paths ) {
	...
	//git archive --format=tar --remote=<remote_name-or-URL> master README
	let remote = ( targetDesc && targetDesc.type !== 'trunk' ) ? targetDesc.name : 'master';
	for ( var i=0; i<paths.length; ++i ) {
		var filepath = paths[i].replace( /\\/g, '/' );
		let result = await executeGit( [ 'archive', '--format=tar', '--remote=' + url, remote, filepath ] );
		if ( result.code !== 0 ) {
			return false;
		}
	}
	...
	// Old method requiring clone of repo for each call
