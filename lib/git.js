...
export async function createRepo( dir, options ) {
	...
	var checkoutDir = misc.getTempDir();
	try {
		// init bare repo
...
export async function createTag( dir, url, targetDesc, tagName, commitLocalChanges, mergeLocalChanges, mergeBowerJson, mergeOriginalBranchTargetDesc, taggedBowerJson, workingCopyRevision, commentPrefix ) {
	let currentBranch = ( await getCurrentTargetDesc( dir ) ).name;
	let name = mergeOriginalBranchTargetDesc ? mergeOriginalBranchTargetDesc.name : 'master';
	...
	await executeGit( [ 'checkout', '-b', tagstargetDesc, workingCopyRevision ], { cwd: dir } );

	let committedRevision;
	if ( commitLocalChanges ) {
		await executeGit( [ 'commit', '-m', commentPrefix + 'Committing local changes', '.' ], { cwd: dir } );
	...
	if ( taggedBowerJson  ) {
		fs.writeFileSync( path.join( dir, 'bower.json' ), JSON.stringify( taggedBowerJson, null, '\t' ) );
		await executeGit( [ 'add', 'bower.json' ], { cwd: dir } );
		await executeGit( [ 'commit', '-m', commentPrefix + 'Adding modified bower.json', 'bower.json' ], { cwd: dir } );
	}
	await executeGit( [ 'tag', '-a', tagName, '-m', commentPrefix + 'Creation', tagstargetDesc ], { cwd: dir } );
	...
	if ( committedRevision ) {
		try {
			...
			if ( mergeBowerJson ) {
				...
				await executeGit( [ 'commit', '-m', commentPrefix + "Merging changes with original branch '" + name + "'" ], { cwd: dir, quiet: true } );
			}
	...
	//	await executeGit( [ 'push', '-u', 'origin', mergeOriginalBranchTargetDesc.name ], { cwd: tempDir } );
	}
	// push changes
	await executeGit( [ 'push', '-u', 'origin', tagName ], { cwd: dir } );
	await executeGit( [ 'checkout', currentBranch ], { cwd: dir } );
	await executeGit( [ 'reset', '--soft', workingCopyRevision ], { cwd: dir } );
}
