...
function cherryPickMerge( tagUrl, commitRevision, url, options ) {
	// mergeOriginalBranchTargetDesc, mergeBowerJson, commentPrefix
	let name = options.mergeOriginalBranchTargetDesc ? options.mergeOriginalBranchTargetDesc.name : 'trunk';
	...
	return checkout( url, options.mergeOriginalBranchTargetDesc, tempDir )
		.then( function() {
			// perform merge
		...
		.then( function() {
			// then commit
			return new Promise( function( resolve, reject ) {
				svn.commands.commit( tempDir, { quiet: true, msg: options.commentPrefix + "Merging changes with original branch '" + options.mergeOriginalBranchTargetDesc.name + "'" }, function( err ) {
					err ? reject( err ) : resolve();
				} );
			} );
		} )
	...
	if ( !options.commitLocalChanges ) {
		...
		return new Promise( function( resolve, reject ) {
			...
			if ( options.taggedBowerJson ) {
				mucc.put( JSON.stringify( options.taggedBowerJson, null, 2 ), joinUrl( url, { type: 'tag', name: tagName }, 'bower.json' ) );
			}
		...
		.then( function() {
			...
			if ( options.taggedBowerJson ) {
				return unCat( url, { type: 'tag', name: tagName }, 'bower.json', JSON.stringify( options.taggedBowerJson, null, 2 ), options.commentPrefix + 'Adding modified bower.json' );
			} else {
				return new Promise( function( resolve ) { resolve(); } ); // no-op
			}
		} )
