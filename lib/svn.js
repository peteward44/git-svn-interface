...
export let name = 'svn';
export let tortoiseExe = 'TortoiseProc.exe';
...
export function unCat( url, targetDesc, filepath, text, msg ) {
	// mucc 'put' does not need file deleted before hand unlike import, so use that instead
	return new Promise( function( resolve, reject ) {
		let mucc = new svn.util.MuccHelper();
		mucc.put( text, joinUrl( url, targetDesc, filepath ) );
		mucc.commit( { msg: msg || 'commit' }, function( err, result ) {
...
function cherryPickMerge( tagUrl, commitRevision, url, mergeOriginalBranchTargetDesc, mergeBowerJson, commentPrefix ) {
	// TODO: delete temp dir
	let name = mergeOriginalBranchTargetDesc ? mergeOriginalBranchTargetDesc.name : 'trunk';
	let tempDir = misc.getTempDir();
	return checkout( url, mergeOriginalBranchTargetDesc, tempDir )
		.then( function() {
			// perform merge
		...
		.then( function() {
			if ( !mergeBowerJson ) {
				// revert bower.json if not specified
				return new Promise( function( resolve, reject ) {
					let bowerJsonPath = path.join( tempDir, 'bower.json' );
					if ( fs.existsSync( bowerJsonPath ) ) {
						svn.commands.revert( bowerJsonPath, function( err ) {
							err ? reject( err ) : resolve();
						} );
					} else {
						resolve();
		...
		.then( function() {
			// then commit
			return new Promise( function( resolve, reject ) {
				svn.commands.commit( tempDir, { quiet: true, msg: commentPrefix + "Merging changes with original branch '" + mergeOriginalBranchTargetDesc.name + "'" }, function( err ) {
					err ? reject( err ) : resolve();
				} );
			} );
		} )
...
export function createTag( dir, url, targetDesc, tagName, commitLocalChanges, mergeLocalChanges, mergeBowerJson, mergeOriginalBranchTargetDesc, taggedBowerJson, workingCopyRevision, commentPrefix ) {
	let branchUrl = joinUrl( url, targetDesc );
	let tagUrl = joinUrl( url, { type: 'tag', name: tagName } );
	let tagComment = commentPrefix + 'creation';
	...
	if ( !commitLocalChanges ) {
		...
		return new Promise( function( resolve, reject ) {
			...
			mucc.cp( branchUrl, tagUrl, { revision: workingCopyRevision } );
			// tag's bower.json
			if ( taggedBowerJson ) {
				mucc.put( JSON.stringify( taggedBowerJson, null, 2 ), joinUrl( url, { type: 'tag', name: tagName }, 'bower.json' ) );
			}
		...
		.then( function() {
			return new Promise( function( resolve, reject ) {
				...
				svn.commands.commit( dir, commentPrefix + 'Committing local changes', function( err, stdout ) {
					err ? reject( err ) : resolve( stdout );
				} );
			} );
		} )
		.then( function( commitStdout ) {
			if ( mergeLocalChanges ) {
				...
				if ( matches && matches.length > 1 ) {
					...
					return cherryPickMerge( tagUrl, commitRevision, url, mergeOriginalBranchTargetDesc, mergeBowerJson, commentPrefix );
				}
			}
			return new Promise( function( resolve ) { resolve(); } ); // no-op
		} )
		.then( function() {
			// commit bower.json to tag
			if ( taggedBowerJson ) {
				return unCat( url, { type: 'tag', name: tagName }, 'bower.json', JSON.stringify( taggedBowerJson, null, 2 ), commentPrefix + 'Adding modified bower.json' );
			} else {
				return new Promise( function( resolve ) { resolve(); } ); // no-op
			}
		} )
...
export async function createBranch( dir, branchName, doSwitch, comment ) {
	...
	await new Promise( function( resolve, reject ) {
		...
		mucc.commit( { msg: comment || 'Create branch' }, function( err ) {
			err ? reject( err ) : resolve();
		} );
	} );
	if ( doSwitch ) {
